...
{    
    [MenuDef(Id = "StationFl", Name = "流速計監測站管理", MenuPath = "即時監控資訊", Action = "Index", Index = 100, Func = FuncEnum.ALL, AllowAnonymous = false)]
    public class StationFlController : Dou.Controllers.APaginationModelController<BasicStt>
    {
        // GET: Country
        ...
        protected override IQueryable<BasicStt> BeforeIQueryToPagedList(IQueryable<BasicStt> iquery, params KeyValueParams[] paras)
        {
            ...
            result = result.Where(x => x.dev_id.Substring(0, 2).ToLower() == "fl").ToList();
        ...
        public override DataManagerOptions GetDataManagerOptions()
        {
            ...
            opts.GetFiled("ref_dev_id").visible = false;
            opts.GetFiled("ref_dev_id").visibleEdit = false;
            opts.GetFiled("sttDev_wdepth").visible = false;
            opts.GetFiled("sttDev_alarm3").visible = false;
            opts.GetFiled("sttDev_alarm2").visible = false;
            opts.GetFiled("sttDev_wdepth").visible = false;
            opts.GetFiled("sttDev_level").title = "流速";
            opts.GetFiled("county_code").filter = false;
            opts.GetFiled("stt_no").filter = false;
